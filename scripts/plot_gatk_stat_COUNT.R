### PLOT STAT FROM GATK REPORT (VARIANTCOUNT) ###
...
#inD="/humgen/atgu1/fs03/wip/aganna/fin_seq/processed/seq/temp/COUNT_gatkreport_G77318RH"
...
d3 <- read.table(paste0(inD,"_INDEL_EX.txt"), header=T, stringsAsFactor=F)
d4 <- read.table(paste0(inD,"_INDEL_NEX.txt"), header=T, stringsAsFactor=F)
...
DPd <- read.csv(paste0(outD,"mediancoverage.csv"))
newdata1DP <- merge(newdata1,DPd,by.x="IDinsdel",by.y="X.1")
newdata1DP <- newdata1DP[order(newdata1DP$meancoverage), ]

newdata2DP <- merge(newdata2,DPd,by.x="IDinsdel",by.y="X.1")
newdata2DP <- newdata2DP[order(newdata2DP$meancoverage), ]


png(paste0(outD,"singeltons_by_sample.png"), width=800, height=800, type="cairo")

p1 <- ggplot(aes(y = single, x=1:nrow(newdata1DP)), data=newdata1DP) + geom_point(size=4,alpha=0.8,aes(color=meancoverage)) +xlab("Samples ordered by DP") + ylab("N. Singlentons") + ggtitle("N. Singletons - SNPs, EXOMES") + geom_text(aes(label=IDsing)) + scale_colour_gradient(low = "blue", high="red")
p2 <- ggplot(aes(y = single, x=1:nrow(newdata2DP)), data=newdata1DP) + geom_point(size=4,alpha=0.8,aes(color=meancoverage)) + scale_colour_gradient(low = "blue", high="red") +xlab("Samples ordered by DP") + ylab("N. Singlentons") + ggtitle("N. Singletons - SNPs, WG") + geom_text(aes(label=IDsing))
